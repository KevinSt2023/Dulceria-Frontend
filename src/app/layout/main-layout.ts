...
<div class="flex h-screen bg-slate-50 overflow-hidden">
  ...
  <aside class="fixed lg:static inset-y-0 left-0 z-30
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-hide">
      ...
      <div class="pt-2">
        ...
        <a *ngIf="isAdmin()" routerLink="/app/reportes"
          routerLinkActive="bg-slate-800 text-white"
          class="nav-item">
          <img src="assets/icon/reporte.png"
              class="w-5 h-5"
      ...
      <div *ngIf="isAdmin()" class="pt-4">
        ...
        <button (click)="toggle('inv')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/inventario.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Gestión
          </span>
      ...
      <div *ngIf="isAdminOrSuper() || isCajero()" class="pt-4">
        ...
        <button (click)="toggle('ventas')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/venta.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Facturación
          </span>
          <span class="text-xs text-slate-500"
        ...
        <div *ngIf="openMenu === 'ventas'" class="pl-4 space-y-0.5 mt-0.5">
          <a *ngIf="isAdmin()" routerLink="/app/pos"          routerLinkActive="text-cyan-400" class="sub-item">Punto de Venta</a>
          <a *ngIf="isAdmin()" routerLink="/app/comprobantes" routerLinkActive="text-cyan-400" class="sub-item">Comprobantes</a>
          <a *ngIf="isSuperAdmin()" routerLink="/app/configuracion_nubefact" routerLinkActive="text-cyan-400" class="sub-item">Fact. Electrónica</a>
          <a *ngIf="isSuperAdmin()" routerLink="/app/series_comprobantes" routerLinkActive="text-cyan-400" class="sub-item">Series de Comprobantes</a>
        </div>        
      </div>

      <!-- PEDIDOS — Vendedor, Admin, SuperAdmin -->
      <div *ngIf="isAdmin() || isVendedor()" class="pt-4">
        <p class="section-label">Pedidos</p>
        <button (click)="toggle('pedidos')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/pedido.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Órdenes
          </span>
          <span class="text-xs text-slate-500"
        ...
        <div *ngIf="openMenu === 'pedidos'" class="pl-4 space-y-0.5 mt-0.5">
          <a routerLink="/app/pedidos"  routerLinkActive="text-cyan-400" class="sub-item">Mis pedidos</a>
          <a routerLink="/app/catalogo" routerLinkActive="text-cyan-400" class="sub-item">Catálogo disponible</a>
        </div>
      </div>

      <!-- PRODUCCIÓN -->
      <div *ngIf="isProduccion() || isAdmin()" class="pt-4">
        <p class="section-label">Producción</p>
        <button (click)="toggle('prod')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/inventario.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Cola
        ...
        <div *ngIf="openMenu === 'prod'" class="pl-4 space-y-0.5 mt-0.5">
          <a routerLink="/app/seguimiento" routerLinkActive="text-cyan-400" class="sub-item">Cola de producción</a>
        </div>
      </div>

      <!-- DISTRIBUCIÓN -->
      <div *ngIf="isDistribuidor() || isAdmin()" class="pt-4">
        <p class="section-label">Distribución</p>
        <button (click)="toggle('dist')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/pedido.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Entregas
        ...
        <div *ngIf="openMenu === 'dist'" class="pl-4 space-y-0.5 mt-0.5">
          <a routerLink="/app/distribucion" routerLinkActive="text-cyan-400" class="sub-item">Panel de distribución</a>
        </div>
      </div>
      ...
      <div *ngIf="isAdmin()" class="pt-4">
        ...
        <button (click)="toggle('conf')" class="nav-item w-full justify-between">
          <span class="flex items-center gap-2">
            <img src="assets/icon/config.png" class="w-5 h-5"
                 style="filter: brightness(0) invert(1) opacity(0.6)"/>
            Sistema
          </span>
          <span class="text-xs text-slate-500"
        ...
        <div *ngIf="openMenu === 'conf'" class="pl-4 space-y-0.5 mt-0.5">
          ...
          <a *ngIf="isSuperAdmin()"
             routerLink="/app/roles"             routerLinkActive="text-cyan-400" class="sub-item">Roles</a>
          <a *ngIf="isSuperAdmin() || isAdmin() "
             routerLink="/app/sucursales"        routerLinkActive="text-cyan-400" class="sub-item">Sucursales</a>
          ...
          <!-- Config. Empresa ELIMINADO — el SuperAdmin usa Panel SuperAdmin -->
        </div>
      </div>
  ...
  <div class="flex-1 flex flex-col overflow-hidden min-w-0">
    ...
    <header class="bg-white border-b border-gray-100 px-4 lg:px-6 py-3 flex items-center gap-4">
      <button (click)="sidebarOpen = true"
              class="lg:hidden p-2 rounded-lg text-gray-500 hover:bg-gray-100">☰</button>

      <div class="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-full">
        <span class="text-xs">🏪</span>
        <span class="text-xs text-slate-600 font-medium max-w-[160px] truncate">{{ sucursalNombre }}</span>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button (click)="logout()"
                class="flex items-center gap-1.5 px-3 py-1.5 text-red-500
    ...
    <main class="flex-1 overflow-auto p-4 lg:p-6">
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6 min-h-full">
  ...
  .section-label {
    ...
    letter-spacing: 0.1em; padding: 0 10px; margin-bottom: 4px; font-weight: 600;
  }
  nav::-webkit-scrollbar { display: none; }
  nav { -ms-overflow-style: none; scrollbar-width: none; }
  .sub-item {
    display: block; padding: 7px 10px; border-radius: 6px;
    font-size: 13px; color: #cbd5e1; transition: all 0.15s;
  }
...
export class MainLayout implements OnInit {
  ...
  toggle(menu: string) { this.openMenu = this.openMenu === menu ? null : menu; }
