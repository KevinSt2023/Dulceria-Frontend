...
<div>
      ...
      <thead class="bg-gray-50">
        <tr>
          <th class="p-3 text-center text-xs font-semibold text-gray-500
                     uppercase tracking-wide w-12">N°</th>
          ...
          <td class="p-3">
            <div class="flex justify-center gap-1.5">
              <button (click)="verDetalle(p)"
                      title="Ver detalle"
                      class="bg-slate-100 hover:bg-slate-200 text-slate-600
                             w-8 h-8 rounded-lg text-sm flex items-center
  ...
  <div *ngIf="mostrarModal"
    ...
    <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl
      ...
      <!-- Stepper -->
      <div class="flex items-center gap-1 px-6 py-3 border-b
        ...
        <div *ngFor="let s of pasos; let i = index"
          ...
          <span [ngClass]="paso >= i + 1 ? 'text-gray-700' : 'text-gray-400'"
                class="text-xs font-medium hidden sm:block mr-1">
            {{ s }}
          </span>
          <div *ngIf="i < pasos.length - 1"
               class="w-8 h-px mx-1"
               [ngClass]="paso > i + 1 ? 'bg-green-400' : 'bg-gray-200'">
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div *ngIf="paso === 1">
          ...
          <div *ngIf="clienteEncontrado"
               class="flex items-center gap-3 bg-green-50 border
                      border-green-200 p-3 rounded-xl mb-4">
          ...
          <div *ngIf="mostrarFormCliente"
               class="border border-yellow-200 bg-yellow-50
                      p-4 rounded-xl">
            <p class="font-semibold text-yellow-700 text-sm mb-3">
              📝 Registrar nuevo cliente
            </p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input [(ngModel)]="nuevoCliente.nombre"
                     placeholder="Nombres y apellidos *"
                     class="p-2.5 border border-yellow-200 rounded-lg
                            text-sm bg-white outline-none
                            focus:ring-2 focus:ring-yellow-400"/>
              <input [value]="dniBusqueda"
                     placeholder="N° Documento"
                     readonly
                     class="p-2.5 border border-yellow-200 rounded-lg
                            text-sm bg-yellow-100 text-gray-500"/>
            ...
            <button (click)="crearCliente()"
                    class="mt-3 bg-yellow-500 hover:bg-yellow-600 text-white
                           px-4 py-2 rounded-lg text-sm font-medium
        ...
        <div *ngIf="paso === 2">
          ...
          <div class="flex gap-2 mb-3">
            <select [(ngModel)]="nuevoItem.producto_id"
              ...
              <option *ngFor="let p of productos"
                      ...
                      [disabled]="!p.permite_pedido_sin_stock &&
            ...
            <input type="number"
                   ...
                   min="1"
                   class="w-20 p-2.5 border border-gray-200 rounded-xl
                          text-center text-sm focus:ring-2
          ...
          <!-- Lista items -->
          <div class="border border-gray-100 rounded-xl bg-gray-50
                      divide-y max-h-52 overflow-y-auto mb-4">
            ...
            <div *ngFor="let d of form.detalles; let i = index"
                 class="flex items-center gap-2 p-3 bg-white">
              ...
              <input type="number"
                     ...
                     class="w-16 border border-gray-200 p-1.5 text-center
              ...
              <button (click)="eliminarItem(i)"
                      class="text-red-400 hover:text-red-600 w-6 h-6
                             flex items-center justify-center rounded
                             hover:bg-red-50 transition-colors text-lg">
                ✕
              </button>
            </div>
          </div>
        ...
        <div *ngIf="paso === 3">
          ...
          <div *ngIf="form.tipos_pedido === 'DELIVERY'" class="space-y-4">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button (click)="seleccionarMetodo('contra_entrega', false)"
                  ...
                  <p class="text-xs text-gray-400">Paga al recibir</p>
                </button>
                ...
                <button (click)="seleccionarMetodo('yape', true)"
                        ...
                        class="border-2 rounded-xl p-3 text-center
                               transition-all hover:border-gray-300">
                  <p class="text-xl mb-1">📱</p>
                  <p class="font-semibold text-xs text-gray-800">Yape / Plin</p>
                  <p class="text-xs text-gray-400">Ya pagó</p>
                </button>
                ...
                <button (click)="seleccionarMetodo('efectivo', true)"
                        ...
                        class="border-2 rounded-xl p-3 text-center
                               transition-all hover:border-gray-300">
                  <p class="text-xl mb-1">💰</p>
                  <p class="font-semibold text-xs text-gray-800">Efectivo</p>
                  <p class="text-xs text-gray-400">Pago en tienda</p>
                </button>
                ...
                <button (click)="seleccionarMetodo('tarjeta', true)"
                        ...
                        class="border-2 rounded-xl p-3 text-center
                               transition-all hover:border-gray-300">
                  <p class="text-xl mb-1">💳</p>
                  <p class="font-semibold text-xs text-gray-800">Tarjeta</p>
                  <p class="text-xs text-gray-400">Pago en tienda</p>
                </button>
              </div>
            </div>
            ...
            <div *ngIf="form.metodo_pago === 'yape' && qrActual"
              ...
              <div class="flex items-center gap-4">
                <img [src]="'data:image/png;base64,' + qrActual.qr_base64"
                     class="w-28 h-28 object-contain rounded-lg border
            ...
            <div *ngIf="form.metodo_pago === 'yape' && !qrActual && !cargandoQR"
              ...
              <p class="text-xs text-yellow-700 text-center">
                ⚠️ QR de Yape no configurado — pide al administrador
                que lo configure en Configuración → Métodos de pago
              </p>
            </div>
            ...
            <div>
              ...
              <div *ngIf="usandoDireccionCliente"
                   class="bg-green-50 border border-green-200 rounded-xl
                          p-3 text-sm">
                ...
                <p *ngIf="!clienteEncontrado?.direccion"
                  ...
                  ⚠️ El cliente no tiene dirección registrada.
                  Usa "Otra dirección".
                </p>
              </div>
          ...
          <div *ngIf="form.tipos_pedido === 'PICKUP'" class="space-y-3">
            ...
            <div>
              ...
              <input [(ngModel)]="form.observaciones"
                     placeholder="Ej: viene a las 3pm, pide sin sal..."
                     class="w-full p-2.5 border border-gray-200 rounded-xl
                            text-sm focus:ring-2 focus:ring-blue-400
                            outline-none"/>
            </div>
...
export class PedidosComponent implements OnInit {
  // ← Todo el TypeScript queda EXACTAMENTE igual que el que tienes
  // Solo cambia el template de arriba
  ...
  nuevoPedido() {
    ...
    this.form = {
      ...
      sucursal_recojo_id: null, pagado: false, metodo_pago: null
    };
    this.entrega = {
      ...
      error: () => {
        ...
        obs.subscribe({
          next: (res: any) => {
            ...
            if (res.nombre) {
              this.nuevoCliente = {
                nombre: res.nombre, documento: res.documento,
                telefono: '', email: '', direccion: res.direccion ?? ''
              };
              ...
              this.mostrarFormCliente = true;
              this.clienteEncontrado  = null;
              Swal.fire('No encontrado', 'Ingresa los datos manualmente', 'warning');
            }
            this.cd.detectChanges();
          },
          error: () => {
  ...
  crearCliente() {
    ...
    this.clientesService.createClientes(this.nuevoCliente).subscribe({
      next: (res: any) => {
        ...
        this.clienteEncontrado  = res;
        this.mostrarFormCliente = false;
        Swal.fire({ icon: 'success', title: 'Cliente registrado',
    ...
    if (!prod.permite_pedido_sin_stock) {
      ...
      if (totalSolicitado > prod.stock_disponible) {
        Swal.fire({ icon: 'warning', title: 'Stock insuficiente',
          html: `Solo hay <b>${prod.stock_disponible}</b> unidad(es) disponibles
                 de "<b>${prod.nombre}</b>".` });
        return;
      }
    }
    ...
    if (this.form.tipos_pedido === 'DELIVERY') {
      if (this.usandoDireccionCliente) {
        ...
        this.form.direccion_entrega = [
          this.entrega.direccion, dist?.nombre, prov?.nombre, dep?.nombre
        ].filter(Boolean).join(', ');
      }
      this.form.sucursal_recojo_id = null;
    } else {
    ...
    this.pedidosService.createPedido(this.form).subscribe({
      next: (res: any) => {
        ...
        Swal.fire({
          ...
          html: res.sucursal_preparacion
                  ? `Se preparará en: <b>${res.sucursal_preparacion}</b>`
      ...
      error: (err) => {
        ...
        if (err.error?.errores) {
          ...
          Swal.fire({
            icon: 'warning',
            title: err.error.mensaje ?? 'Stock insuficiente',
            html: `<ul style="text-align:left">${lista}</ul>
                   <p style="color:#6b7280;font-size:13px;margin-top:8px">
                     ${err.error.sugerencia ?? ''}
                   </p>`,
            confirmButtonText: 'Entendido'
          });
        } else {
          Swal.fire('Error',
            err?.error?.message ?? err?.error ?? 'No se pudo crear', 'error');
        }
      }
    });
  }
  ...
  verDetalle(p: any) {
    if (!p.detalles?.length) {
      Swal.fire('Sin detalle', 'Este pedido no tiene productos', 'info');
      return;
    }
    const rows = p.detalles.map((d: any) =>
      `<tr>
        <td style="padding:6px 8px;text-align:left">${d.producto}</td>
        <td style="padding:6px 8px;text-align:center">${d.cantidad}</td>
        <td style="padding:6px 8px;text-align:right">
          S/ ${Number(d.precio).toFixed(2)}
        </td>
        <td style="padding:6px 8px;text-align:right;font-weight:600;color:#16a34a">
    ...
    Swal.fire({
      ...
      html: `
        <p style="font-size:12px;color:#6b7280;margin-bottom:8px">
          ${p.tipos_pedido === 'DELIVERY' ? '🛵 Delivery' : '🏪 Pickup'}
          ${p.direccion_entrega ? ' · ' + p.direccion_entrega : ''}
        </p>
  ...
  cancelarPedido(p: any) {
    Swal.fire({
      ...
      this.pedidosService.cancelarPedido(p.pedido_id).subscribe({
        next: () => this.cargar(),
        error: (err) =>
          Swal.fire('Error', err?.error || 'No se pudo cancelar', 'error')
      });
    });
  }
